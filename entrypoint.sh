...
sendComment() {
    ...
    local GITHUB_PULL_REQUEST_USER="$3"
    ...
    curl -sSL \
         ...
         -X POST \
         -H "Content-Type: application/json" \
         -d "{\"body\":\"@${GITHUB_PULL_REQUEST_USER}, ${GITHUB_ISSUE_COMMENT}\"}" \
...
main() {
    ...
    if [[ "$GITHUB_EVENT_NAME" == "pull_request" && "$GITHUB_EVENT_ACTION" == "opened" ]]; then
        ...
        GITHUB_PULL_REQUEST_EVENT_TITLE=$(jq --raw-output .pull_request.title "$GITHUB_EVENT_PATH")
        GITHUB_PULL_REQUEST_USER=$(jq --raw-output .pull_request.user.login "$GITHUB_EVENT_PATH")

        if [[ "$GITHUB_PULL_REQUEST_EVENT_BODY" =~ $PULL_REQUEST_PATTERN ]] || [[ "$GITHUB_PULL_REQUEST_EVENT_TITLE" =~ $PULL_REQUEST_PATTERN ]]
        then
